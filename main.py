...
#     @classmethod
#     def clean_currency(cls, v):
#         if not v or str(v).strip() in ["-", "", "null", "N/A"]: return 0.0
#         if isinstance(v, str):
...
#     @classmethod
#     def clean_volume(cls, v):
#         if not v or str(v).strip() in ["-", "", "null", "N/A"]: return 0
#         if isinstance(v, str):
...
#             try: await client.post(self.url, json={"chat_id": self.chat_id, "text": message, "parse_mode": "Markdown"})
...
# # --- CORE ENGINE ---
# class NGXEngine:
#     # Essential for Stooq fallback to ensure we hit high-value targets
#     STOOQ_TICKERS = [
#             # Banking & Financials
...
#             # Consumer Goods
#             "NESTLE", "NB", "GUINNESS", "BUAFOODS", "DANGSUGAR", "NASCON", 
#             "FLOURMILL", "CADBURY", "UNILEVER", "HONYFLOUR", "VITAFOAM", "PZ",
            
#             # Energy & Oil/Gas
#             "SEPLAT", "OANDO", "CONOIL", "TOTAL", "MRS", "ETERNA",
            
#             # Conglomerates & Services
#             "TRANSCORP", "UACN", "JAIZBANK", "JULI", "CAVERTON", "RTBRISCOE",
            
#             # Insurance & Others
#             "AIICO", "MANSARD", "NEM", "CORNERST", "NGXGROUP", "WAPIC", "AFRIPRUD", "MTN"
#         ]
#     def __init__(self):
...
#         self.wa = WhatsAppNotifier()
#         self.tg = TelegramNotifier()
#         self.proxy = os.getenv("NG_PROXY_URL")
#         self.headers = {"User-Agent": "Mozilla/5.0", "Referer": "https://ngxgroup.com/"}
#         Base.metadata.create_all(self.engine)
...
#         print("🌐 Trying Stooq (Direct)...")
#         stooq_data = []
#         async with self._make_client(use_proxy=False) as client:
#             for symbol in self.STOOQ_TICKERS:
#                 try:
#                     url = f"https://stooq.com/q/d/l/?s={symbol.lower()}.ng&d1={target_date.strftime('%Y%m%d')}&i=d"
#                     res = await client.get(url)
#                     if "Date" in res.text and len(res.text.splitlines()) > 1:
#                         p = res.text.splitlines()[1].split(",")
#                         stooq_data.append({"Symbol": symbol, "Open": p[1], "High": p[2], "Low": p[3], "Close": p[4], "Volume": p[5]})
#                 except: continue
#         if stooq_data:
#             path = f"stooq_{target_date}.json"
#             with open(path, "w") as f: json.dump(stooq_data, f)
#             return path

#         # --- STAGE 2: NGX REST API (Proxied)[cite: 2] ---
#         if self.proxy:
#             print("🔗 Trying NGX API (Proxied)...")
#             async with self._make_client(use_proxy=True) as client:
#                 try:
#                     url = "https://doclib.ngxgroup.com/REST/api/statistics/equities/?pageSize=300"
#                     res = await client.get(url)
#                     if res.status_code == 200:
#                         path = f"ngx_api_{target_date}.json"
#                         with open(path, "w") as f: json.dump(res.json(), f)
#                         return path
#                 except: pass
#         return None
...
#         if "Symbol" in str(data): # NGX API Format
#             for r in data:
#                 sym = r.get("Symbol", "").strip()
#                 if not sym: continue
#                 cp = r.get("ClosePrice") or r.get("PrevClosingPrice") or 0.0
#                 stocks.append(StockSchema(symbol=sym, company_name=r.get("Company2", sym), open_price=r.get("OpeningPrice") or cp, high_price=r.get("HighPrice") or cp, low_price=r.get("LowPrice") or cp, close_price=cp, volume=r.get("Volume") or 0, trade_date=trade_date))
#         else: # Stooq Format
#             for r in data:
#                 stocks.append(StockSchema(symbol=r["Symbol"], company_name=r["Symbol"], open_price=r["Open"], high_price=r["High"], low_price=r["Low"], close_price=r["Close"], volume=r["Volume"], trade_date=trade_date))
#         return stocks
...
#                                ON CONFLICT (symbol, trade_date) DO UPDATE SET close_price=EXCLUDED.close_price, volume=EXCLUDED.volume, percent_change=EXCLUDED.percent_change;""")
#                 session.execute(stmt, s.model_dump(exclude={'old_resistance', 'old_support', 'vol_increase', 'is_corporate_action'}))
#             session.commit()
#         finally: session.close()

#     def get_market_alerts(self, stocks: List[StockSchema]):
#         # Your Original Alert Logic[cite: 2]
#         session = self.Session()
#         breakouts, breakdowns, momentum, spikes = [], [], [], []
#         try:
#             for s in stocks:
#                 s.is_corporate_action = abs(s.percent_change) > 10.5
#                 levels = session.execute(text("SELECT MAX(high_price), MIN(low_price) FROM stock_prices WHERE symbol=:sym AND trade_date < :t"), {"sym": s.symbol, "t": s.trade_date}).fetchone()
#                 if levels and levels[0]:
#                     res, sup = float(levels[0]), float(levels[1])
#                     if s.close_price > res: s.old_resistance=res; breakouts.append(s)
#                     if s.close_price < sup and not s.is_corporate_action: s.old_support=sup; breakdowns.append(s)
                
#                 if s.percent_change >= 5.0 and not s.is_corporate_action: momentum.append(s)

#                 avg_vol = session.execute(text("SELECT AVG(volume) FROM (SELECT volume FROM stock_prices WHERE symbol=:sym ORDER BY trade_date DESC LIMIT 10) as sub"), {"sym": s.symbol}).scalar()
#                 if avg_vol and s.volume > (float(avg_vol) * 2):
#                     s.vol_increase = round(s.volume / float(avg_vol), 1)
#                     spikes.append(s)
#             return breakouts, breakdowns, momentum, spikes
#         finally: session.close()

#     async def send_daily_recap(self, stocks, breakouts, breakdowns, momentum, spikes):
#         # Your Original Notification Format[cite: 2]
...
# if __name__ == "__main__":
#     async def main():
#         engine = NGXEngine(); today = datetime.now().date()
#         path = await engine.download_report(today)
#         if path:
#             stocks = engine.parse_source(path, today)
#             if stocks:
#                 engine.save(stocks)
