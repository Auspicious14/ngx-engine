...
class NGXEngine:
    ...
    async def download_report(self, target_date: date):
        """Attempts to find the report using a multi-stage strategy."""
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            
            # --- STAGE 1: Discovery via Latest Redirects (Most Reliable) ---
            # These endpoints usually serve the most recent PDF generated by the NGX system
            latest_endpoints = [
                "https://ngxgroup.com/ngx-download/daily-official-list-equities/",
                "https://ngxgroup.com/ngx-download/market-data-pricelist-2/"
            ]
            
            for url in latest_endpoints:
                try:
                    res = await client.get(url)
                    if res.status_code == 200 and b"%PDF" in res.content[:4]:
                        path = f"ngx_latest_{target_date.strftime('%Y-%m-%d')}.pdf"
                        with open(path, "wb") as f: f.write(res.content)
                        print(f"✅ Found current report via redirect discovery.")
                        return path
                except Exception: continue

            # --- STAGE 2: Brute-Force Guessing (Fallback) ---
            print("🔍 Stage 1 failed. Falling back to date-string guessing...")
            delimiters = ["-", " ", "", "."]
            ...
            for attempt_str in candidate_strings:
                ...
                # Try both Title Case and UPPER CASE variations
                patterns = [
                    f"Daily%20Official%20List%20-%20Equities%20for%20{encoded_date}.pdf",
                    f"DAILY%20SUMMARY%20FOR%20{encoded_date}.pdf",
                    f"Daily%20Summary%20for%20{encoded_date}.pdf"
                ]
                
                for pattern in patterns:
                    url = f"https://doclib.ngxgroup.com/DownloadsContent/{pattern}"
                    try:
                        res = await client.get(url)
                        if res.status_code == 200:
                            path = f"ngx_manual_{target_date.strftime('%Y-%m-%d')}.pdf"
                            with open(path, "wb") as f: f.write(res.content)
                            print(f"✅ Found report via guessing: {url}")
                            return path
                    except Exception: continue
        ...
        try:
            with pdfplumber.open(path) as pdf:
                for page in pdf.pages:
                    ...
                    for i, row in enumerate(table[:10]): # Check deeper into the page
                    ...
                    for row in table[header_idx + 1:]:
                        ...
                        # Ensure we are in a stock row (Capitalized symbol, no spaces)
                        if not symbol.isupper() or " " in symbol: continue
                        
                        # Close price is usually Col 5 (0-indexed)
                        close_p = row[5]
    ...
    async def send_daily_recap(self, stocks, breakouts, breakdowns, momentum, spikes):
        ...
        if breakouts:
            ...
            for s in breakouts[:3]: msg += f"• *{s.symbol}*: ₦{s.close_price} (Broke ₦{s.old_resistance})\n"
            msg += "\n"

        if momentum:
            msg += "🔥 *HIGH MOMENTUM (5%+)*\n"
            for s in momentum[:3]: msg += f"• *{s.symbol}*: +{s.percent_change:.2f}%\n"
            msg += "\n"

        if spikes:
            msg += "🔊 *UNUSUAL VOLUME*\n"
            for s in spikes[:3]: msg += f"• *{s.symbol}*: {s.vol_increase}x Normal Vol\n"
            msg += "\n"
...
if __name__ == "__main__":
    async def run_daily_sync():
        ...
        if not pdf:
            print(f"🛑 No report found for {today} after trying all strategies.")
            return
